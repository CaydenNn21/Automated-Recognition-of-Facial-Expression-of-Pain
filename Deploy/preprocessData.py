...
class preprocess():
    ...
    def normalized_img(self):
        ...
        all_frames32 = [cv.cvtColor(img, cv.COLOR_BGR2GRAY) for img in all_frames32]
        ...
        # Create Gabor kernel
        gabor_kernel = cv.getGaborKernel((ksize, ksize), sigma, theta, lambd, gamma)
        all_frames32 = [cv.filter2D(input_image, cv.CV_8U, gabor_kernel) for input_image in all_frames32]
        all_frames_1 = np.array(all_frames32, dtype="float32")/ 255.0
